...
「A からアクセスする」「B からアクセスする」を、順に押してみてください。
...
![ブラウザ A の言葉がサーバーを経由してブラウザ B に届く。線はつなぎっぱなし](./images/04-websocket.svg)
...
**同じ言葉が 2 回運ばれている**わけです。デモの「送る」で、その 2 回ぶんが見えます。

## ④ WebRTC — 相手のブラウザへ直接

最後の 1 つが、**真ん中を外す**やり方です。

![あなたのブラウザ A と相手のブラウザ B が直接つながっている](./images/05-p2p.svg)
...
[02 章 02 節](../../02-design/02-blueprint/LECTURE.md) で ④ を選びました。

## 並べた 4 つのうち、3 つはもう通ってきた

別々のものとして並べましたが、実は ② 以外は、この教材を進めるだけで**自分で踏んできました**。

![ローカル・静的サイト・WebSocket・WebRTC を順に通ってきた道のり。動的サイトだけ欠けている](./images/06-journey.svg)

_図: ③ だけは自分では書かない。それでも確かに使っている。_

③ の WebSocket は、`new Peer(...)` を呼んだ瞬間に PeerJS が裏で張っていました。
自分では 1 行も書いていません。
② の動的サイトだけは出てきません。このお絵かきは、どこにも保存しないからです。
...
**住所を持たないブラウザ同士を、どうやって直接つないでいるのか。**
次の節で、その中身を見ます。
