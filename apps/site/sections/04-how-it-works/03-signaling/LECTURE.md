...
名前が似ているので混ざりやすいのですが、**STUN は教えるだけ、TURN は運ぶ**。
別の係です。
...
鏡のようなものです。返ってきた住所が、そのまま `srflx` 候補になります。
...
ただし TURN を通ったときは、**もう直接つながっていません**。
速さの利点も、サーバー代がかからない利点も、ここで失われます。
...
> シグナリング接続 → SDP 交換 → ICE 候補交換 → 接続性チェック → DTLS ハンドシェイク → データチャネル

名前が固いので、それぞれが何をしているかを先に並べておきます。

| 手続き              | やっていること                                                              |
| ------------------- | --------------------------------------------------------------------------- |
| シグナリング接続    | シグナリングサーバーに WebSocket でつなぎ、id を名簿に載せてもらう          |
| SDP 交換            | 「どう話すか」の取り決め（`offer` / `answer`）を、サーバー越しに 1 往復する |
| ICE 候補交換        | 「どこに届けるか」の候補（`host` / `srflx` / `relay`）を集めて、送り合う    |
| 接続性チェック      | 候補の組み合わせに実際にパケットを投げて、通る道を 1 本決める               |
| DTLS ハンドシェイク | 決まった道の上で鍵を交換し、SDP で伝え合った証明書の指紋と突き合わせる      |
| データチャネル      | SCTP のデータチャネルが開く。ここでやっと `conn.send()` が届く              |

直接つながらないときは、接続性チェックが全滅したあとに **TURN 割り当て**
（`Allocate` で中継用の住所を借り、`CreatePermission` で相手を指定する）が挟まります。
