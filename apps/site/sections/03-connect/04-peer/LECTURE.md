...
これが P2P の一番の難所で、この章のあとの
[04 章 02 節](../../04-how-it-works/02-webrtc/LECTURE.md)で扱います。
...
ただし、PeerJS でも 1 つだけ省けないものがあります。**最初のひと声**です。
まだつながっていない相手に最初の合図を届けるために、両方が知っている共通の場所を
1 か所だけ経由します。その場所を PeerJS では **PeerServer** と呼び、
公式が無料で開放しているものが **PeerJS Cloud** です。

![PeerJS Cloud は登録もキーもいらない](./images/02-peerjs-cloud.svg)

_図: 挨拶を送ると、名前が返ってくる。_

うれしいことに、PeerJS Cloud は **アカウント登録もキーの発行も不要**です。
接続先は `0.peerjs.com` の 443 番ポートで、ライブラリの中に最初から書いてあります。
だからこのあとやるのは「用意する」ことではなく、`new Peer()` と書くことだけです。

## つながるか、先に確かめる

コードを書く前に、**この回線から PeerJS Cloud に届くか**だけ見ておきます。
下のボタンを押して、**名前が返ってきたら成功**です。

::preview[押すと PeerJS Cloud につないで、配られた名前を表示します]{demo="peer-check" height="220"}

`つながりました。あなたに配られた名前: 8f3a1c9e-...` のような文字列が出れば大丈夫です。
名前を指定せずに `new Peer()` と書くと、PeerJS Cloud が**世界中で重複しない名前**を
1 つ配ってくれます。この配られた名前は、このあともう一度出てきます。
...
![ホストは名乗って待ち、ゲストは名乗らず呼び出す。open からは対等になる](./images/03-host-guest.svg)
